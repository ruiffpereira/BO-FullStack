...
- **End-to-end (`tests/e2e/`, Playwright):** fluxos reais no browser (Chromium). Correr: **`pnpm test:e2e`** — não precisa de nada ligado à mão. Specs: admin, agenda, **agenda-pagamentos**, auth, clientes, conteudos, dashboard, despesas, errors, financeiro, ginasio, loja, **rbac**, **isolamento**, security. Page objects em `tests/e2e/pages/`.
