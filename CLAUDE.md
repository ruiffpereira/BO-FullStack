...
- `/perfil` (T3.3) é core mas também **não** aparece na sidebar — acede-se pelo `AvatarMenu` no topbar; entra em `guardRoots` a par de `/despesas` pela mesma razão (senão o guard por prefixo expulsava-o para o dashboard).
...
|------------|-----------|
| `Shell.tsx` | Layout principal: sidebar, topbar, notification bell + **`BillingBanner`** (acima do `<main>`) + **`AvatarMenu`** (T3.3 — dropdown do avatar no topbar, portal como o flyout do `NavItemGroup`: "O meu perfil" → `/perfil` · "Terminar sessão") |
