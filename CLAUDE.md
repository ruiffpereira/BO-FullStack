...
| `Faturacao.tsx` | `/faturacao` | **core** (todos) | **Platform billing** (a subscrição da plataforma do próprio tenant; brief/tarefas em `.design/platform-billing/` + `.design/self-serve/`). Lê `useGetBillingSubscription` (`GET /api/billing/subscription` → `{ status, modules[], monthlyTotalEur, readOnly, reason, graceEndsAt, … }`; `reason` ∈ none/trialing/**trial_expired**/active/incomplete/grace/past_due_locked/canceled) e mostra o plano (módulos + total/mês), o aviso de estado e o CTA de pagamento. O botão **Gerir/Regularizar pagamento** abre o **Stripe Billing Portal** via `usePostBillingPortal` (`POST /api/billing/portal` → redirect; 409 = ainda sem cliente Stripe → toast "fala com o suporte"). **`trial_expired`** (self-serve, T9 — trial local do signup sem cartão, expirado): alerta vermelho (`role=alert`) que explica (esta fase ainda não cobra automaticamente) e aponta ao **chat de suporte** (`/mensagens`) em vez do portal Stripe. Badges/labels e formatos partilhados em `src/lib/billingStatus.ts` |
