...
import {
  useGetProducts,
  getProductsQueryKey,
} from "../gen/backoffice/hooks/useGetProducts.js";
...
} from "../gen/backoffice/hooks/useGetCategories.js";
...
import {
  useGetOrders,
  getOrdersQueryKey,
} from "../gen/backoffice/hooks/useGetOrders.js";
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <GuardButton onClick={() => onSave(form)} disabled={isBusy || !form.name.trim()}>
            {isPending ? (
              <>
...
export function Loja({ view }: { view: LojaView }) {
  ...
  const invalidateProducts = () =>
    qc.invalidateQueries({ queryKey: getProductsQueryKey() });
  const invalidateOrders = () =>
    qc.invalidateQueries({ queryKey: getOrdersQueryKey() });
  const invalidateCats = () =>
    qc.invalidateQueries({ queryKey: getCategoriesQueryKey() });
      ...
      putCategory.mutate(
        {
          id: transModal.id,
          data: {
            name: transModal.name,
            translations: transModal.translations,
          } as any,
        },
        {
          onSuccess: () => {
          ...
          onError: () => toast.error("Erro ao guardar traduções"),
        },
      );
    } else {
      putSubcategory.mutate(
        {
          ...
          data: {
            name: transModal.name,
            categoryId: transModal.categoryId!,
            translations: transModal.translations,
          } as any,
        },
        {
  ...
  const stats = useMemo(
    () => ({
      ...
      baixo: products.filter((p) => (p.stock ?? 0) > 0 && (p.stock ?? 0) <= 8)
        .length,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <Card key={s.l} className="p-4 flex items-center gap-3">
            <span
              className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${BADGE_TONES[s.t]}`}
            >
          ...
          {loadingProds ? (
            ...
            <Card>
              <EmptyState
                icon="search"
                title="Sem produtos"
                desc="Não há produtos que correspondam à pesquisa."
              />
            </Card>
          )}
        </div>
      ...
      {tab === "encomendas" && (
        <div>
          <Card className="p-4 mb-4 flex items-center gap-3 w-fit">
            <span
              className={`w-10 h-10 rounded-lg flex items-center justify-center ${BADGE_TONES.green}`}
            >
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {pgOrders.pageItems.map((o) => (
                      ...
                      >
                        <td className="px-4 sm:px-5 py-3.5 font-mono text-xs text-zinc-500">
                          {o.orderId.slice(0, 8)}…
                        </td>
                        <td className="px-4 py-3.5 font-semibold text-zinc-900 dark:text-white tabular-nums">
                          {fmtEur(Number(o.price))}
                        </td>
        ...
        <div className="max-w-xl space-y-3">
          ...
          {newCatVisible ? (
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={putCategory.isPending}
              >
                Criar
              </Button>
              <Button
              ...
              <div className="flex items-center gap-2">
                ...
                {editCat?.id === cat.categoryId ? (
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") saveCat(cat.categoryId);
                ...
                {editCat?.id === cat.categoryId ? (
                  <>
                    <Button
                      size="sm"
                      onClick={() => saveCat(cat.categoryId)}
                      disabled={putCategory.isPending}
                    >
                      Guardar
                    </Button>
                  ...
                  <>
                    <button
                      onClick={() =>
                        setEditCat({ id: cat.categoryId, name: cat.name })
                      }
                    ...
                    <button
                      onClick={() =>
                        setTransModal({
                          id: cat.categoryId,
                          name: cat.name,
                          translations: (cat as any).translations ?? {},
                          type: "cat",
                        })
                      }
                  ...
                  >
                    ...
                    {editSub?.id === sub.subcategoryId ? (
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            saveSub(sub.subcategoryId, cat.categoryId);
                    ...
                    {editSub?.id === sub.subcategoryId ? (
                      <>
                        <Button
                          ...
                          disabled={putSubcategory.isPending}
                        >
                          Guardar
                        </Button>
                        <Button
                ...
                {newSubCatId === cat.categoryId ? (
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") saveSub(null, cat.categoryId);
                        if (e.key === "Escape") {
                          setNewSubCatId(null);
                          setNewSubName("");
                        }
                    ...
                    <Button
                      size="sm"
                      onClick={() => saveSub(null, cat.categoryId)}
                      disabled={putSubcategory.isPending}
                    >
                      Criar
                    </Button>
          ...
          {categories.length === 0 && !newCatVisible && (
            <p className="text-sm text-zinc-400 py-12 text-center">
              Sem categorias ainda. Cria a primeira!
            </p>
          )}
        </div>
      )}
      ...
      {transModal && (
        <Modal
          ...
          footer={
            <>
              <Button variant="ghost" onClick={() => setTransModal(null)}>
                Cancelar
              </Button>
              <Button
                onClick={saveTranslations}
                disabled={putCategory.isPending || putSubcategory.isPending}
              >
                Guardar
              </Button>
            </>
        ...
        >
          <TranslationInputs
            ...
            onChange={(translations) =>
              setTransModal((m) => (m ? { ...m, translations } : null))
            }
            fields={["name"]}
