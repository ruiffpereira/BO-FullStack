  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="Margem bruta" icon="trend" tone="green" loading={isLoading} value={fmtEur(data?.margem.total ?? 0)} sub={`${data?.margem.marginPct ?? 0}% de margem`} />
        <KpiCard label="Valor médio por compra" icon="star" tone="blue" loading={isLoading} value={fmtEur(data?.valorMedioPorCompra ?? 0)} />
        <KpiCard label="LTV do cliente" icon="users" tone="violet" loading={isLoading} value={fmtEur(data?.customers.ltv ?? 0)} sub="receita média / cliente" />
        <KpiCard label="Taxa de recompra" icon="cart" tone="amber" loading={isLoading} value={`${data?.customers.taxaRecompra ?? 0}%`} sub="clientes com ≥2 compras" />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <KpiCard label="Perdidos" icon="ban" tone="red" loading={isLoading} value={String(data?.customers.perdidos ?? 0)} sub={`sem comprar há +${data?.customers.lostAfterDays ?? 90}d`} />
      </div>

      {/* Inventário + carrinhos */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="p-5">
          <SectionTitle>Stock parado</SectionTitle>
        ...
        <Card className="p-5">
          <SectionTitle>Carrinhos abandonados</SectionTitle>
        ...
        <Card className="p-5">
          <SectionTitle>Devoluções e descontos</SectionTitle>
